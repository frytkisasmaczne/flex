...
public class PlayerScript : Script
{
    ...
    public float SprintAccelerate = 15000;
    public float AirAccelerate = 10000;
    public float MaxVelocityGround = 400;
    public float MaxVelocitySprint = 1200;
    ...
    public override void OnUpdate()
    {
        ...
        if (Input.GetAction("Fire"))
        {
            ...
            ball.LinearVelocity = CameraTarget.Direction * 1200.0f;
            Destroy(ball, 5.0f);
        }
        ...
        if (Input.GetAction("AltFire"))
        {
            ...
            {
                if (Physics.RayCast(CameraTarget.Position, CameraTarget.Direction, out RayCastHit hit, float.MaxValue, Layers))
                {
                    
                    joint.Target = hit.Collider.Parent;
                }
            }
        }
    }
    ...
    public override void OnFixedUpdate()
    {
        ...
        var sprint = Input.GetAction("Sprint");
        _horizontal = 0;
        _vertical = 0;
        ...
        {
            velocity = MoveGround(velocity.Normalized, Horizontal(_velocity), sprint);
            velocity.Y = -Mathf.Abs(Physics.Gravity.Y * 0.5f);
        }
        else
        {
            velocity = MoveAir(velocity.Normalized, Horizontal(_velocity), sprint);
    ...
    private Vector3 MoveGround(Vector3 accelDir, Vector3 prevVelocity, bool sprint)
    {
        // Apply Friction
        ...
        var accelerate = sprint ? SprintAccelerate : GroundAccelerate;
        var maxVelocity = sprint ? MaxVelocitySprint : MaxVelocityGround;
        // GroundAccelerate and MaxVelocityGround are server-defined movement variables
        return Accelerate(accelDir, prevVelocity, accelerate, maxVelocity);
    }

    private Vector3 MoveAir(Vector3 accelDir, Vector3 prevVelocity, bool sprint)
    {
        var accelerate = sprint ? SprintAccelerate : AirAccelerate;
        var maxVelocity = sprint ? MaxVelocitySprint : MaxVelocityAir;
